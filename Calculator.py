...
class CoordinatesCalculator:
    ...
    def __transform_coordinate_to_float(coordinate):
        coord = coordinate['degree'] + (coordinate['minute'] + coordinate['second'] / 60) / 60
        return coord

    @staticmethod
    def __transform_float_to_coordinate(float):
        coord = {
            "degree": int(float),
            "minute": int(float % 1 * 60),
            "second": round(float % 1 * 60 % 1 * 60, 2)
        }
        return coord
    ...
    def __get_center(coordinates):
        ...
        x_mid = round((x_top - x_bot)/2 + x_top, 3)
        y_mid = round((y_top - y_bot)/2 + y_top, 3)
        return {
            "x": x_mid,
            "y": y_mid
        }
